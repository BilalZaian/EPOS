...
# in order for this to work wrappers.py has to be adjusted
# added check of free_blocks
class Test_Rem:
    ...
    def test_rem_wrong_name(self):
        fs = setup(5)
        fs = set_dir(name="firstDir",inode=1,parent=0,parent_block=0,fs=fs)
        retval = libc.fs_rm(ctypes.byref(fs), ctypes.c_char_p(bytes("firstDir","UTF-8")))
        assert retval == -1
        retval = libc.fs_rm(ctypes.byref(fs), ctypes.c_char_p(bytes("idkasdf","UTF-8")))
        assert retval == -1
        retval = libc.fs_rm(ctypes.byref(fs), ctypes.c_char_p(bytes("/idkasdf","UTF-8")))
        assert retval == -1
        retval = libc.fs_rm(ctypes.byref(fs), ctypes.c_char_p(bytes("/firstDir","UTF-8")))
