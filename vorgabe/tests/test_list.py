...
class Test_List:
    ...
    def test_list_nested_multiple(self):
        fs = setup(10)
        fs = set_dir(name="parent",inode=1,parent=0,parent_block=0,fs=fs) # /parent
        fs = set_dir(name="parent",inode=2,parent=1,parent_block=0,fs=fs) # /parent/parent
        libc.fs_list.restype = ctypes.c_char_p
        retval = libc.fs_list(ctypes.byref(fs), ctypes.c_char_p(bytes("/parent","UTF-8")))
        assert retval.decode("utf-8") == "DIR parent\n"
        fs = set_dir(name="par",inode=3,parent=1,parent_block=1,fs=fs) # /parent/par (dir)
        fs = set_fil(name="par",inode=4,parent=1,parent_block=2,fs=fs) # /parent/par (fil)
        retval = libc.fs_list(ctypes.byref(fs), ctypes.c_char_p(bytes("/parent","UTF-8")))
        assert retval.decode("utf-8") == "DIR parent\nDIR par\nFIL par\n"
        fs = set_fil(name="parent",inode=5,parent=1,parent_block=3,fs=fs) # /parent/parent
        fs = set_dir(name="idk",inode=6,parent=1,parent_block=4,fs=fs) # /parent/idk
        retval = libc.fs_list(ctypes.byref(fs), ctypes.c_char_p(bytes("/parent","UTF-8")))
        assert retval.decode("utf-8") == "DIR parent\nDIR par\nFIL par\nFIL parent\nDIR idk\n"
    def test_list_multiple_not_in_order(self):
        fs = setup(10)
        fs = set_dir(name="Dir1",inode=1,parent=0,parent_block=0,fs=fs)
        fs = set_dir(name="Dir2",inode=2,parent=0,parent_block=3,fs=fs)
        fs = set_dir(name="Dir3",inode=3,parent=0,parent_block=2,fs=fs)
        fs = set_fil(name="Fil1",inode=4,parent=0,parent_block=4,fs=fs)
        fs = set_fil(name="Fil2",inode=5,parent=0,parent_block=1,fs=fs)
